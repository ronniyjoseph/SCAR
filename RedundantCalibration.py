...
def LogcalSolver(amp_pinv,phase_pinv,obs_visibilities):	
	...
	phase_log[len(phase_log)-4:len(phase_log)-1] = 0.
...
def LincalMatrixPopulator(uv_positions,correlation_obs,gain_0,visibility_0,red_tiles,red_groups):
    ...
    for i in range(len(uv_positions)):
        ...
        A_matrix[2*i,2*(len(red_tiles)+index_group)+1] = -numpy.imag(gain_0[index1]*numpy.conj(gain_0[index2]))
        ...
        A_matrix[2*i+1,2*(len(red_tiles)+index_group)] = numpy.imag(gain_0[index1]*numpy.conj(gain_0[index2]))
        A_matrix[2*i+1,2*(len(red_tiles)+index_group)+1] = numpy.real(gain_0[index1]*numpy.conj(gain_0[index2]))
...
def LincalSolver(uv_positions,correlation_obs,amp_solutions, phase_solutions,red_tiles, red_groups):
    ...
    while d_corrections > 1e-9 and counter < 1000:    
        ...
        A_pinv, d_correlation = LincalMatrixPopulator(uv_positions,correlation_obs,gain_0,visibility_0,red_tiles,red_groups)
        ...
        visibility_1 =visibility_0 + (d_solutions_1[2*len(red_tiles)::2] + 1j*d_solutions_1[2*len(red_tiles)+1::2])
